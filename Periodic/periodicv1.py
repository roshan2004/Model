...
rows = 6 #Even number of rows for now, but it can be changed
columns = 6 # Even number of columns for now, but it can be changed
positions = []
dist = 0
for i in range(rows):
    if i % 2 == 0:
...
impropers = np.vstack(
    (get_angles(hexagons), get_between_hexagons_angles(hexagons, int(columns)+1)))
